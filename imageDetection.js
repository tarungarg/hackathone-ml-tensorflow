...
INPUT_1.addEventListener('change', (e) => {
   const text = document.getElementById("data-collector-1").innerHTML;
   document.getElementById("data-collector-1").innerHTML =  e.target.value;
});

INPUT_2.addEventListener('change', (e) => {
  const text = document.getElementById("data-collector-2").innerHTML;
  document.getElementById("data-collector-2").innerHTML = e.target.value;
});
...
async function loadMobileNetFeatureModel() {
  ...
  STATUS.innerText = "AI ready to predict now";
...
function enableCam(VIDEO_ELEMENT, fn) {
  ...
  // console.log("Vide", VIDEO);
    if((VIDEO_ELEMENT === VIDEO)) {
      changeVisibility(VIDEO, "visible");
      videoPlaying_1 = true;
      changeVisibility(VIDEO_1, "hidden");
      VIDEO_1.pause();
    } else if(VIDEO_ELEMENT === VIDEO_1) {
      videoPlaying_2 = true;
      VIDEO.pause();
      changeVisibility(VIDEO_1, "visible");
      changeVisibility(VIDEO, "hidden");
    } else {
      VIDEO.pause();
      VIDEO_1.pause();
      changeVisibility(VIDEO, "hidden");
      changeVisibility(VIDEO_1, "hidden");
      changeVisibility(VIDEO_2, "visible");
      videoPlaying_3 = true;
    }
...
function dataGatherLoop() {
  ...
  // console.log(CLASS_NAMES)
    ...
    if(TRAIN_BUTTON.disabled && (examplesCount[0] > 0 && examplesCount[1] > 0 ))
